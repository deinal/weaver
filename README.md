...
# install s3fs if data is stored in an s3 bucket
pip install s3fs
...
An ONNX model can be served using Triton, which requires a [model configuration file](https://github.com/triton-inference-server/server/blob/main/docs/model_configuration.md) in protobuf format. This can be generated by using the `--triton-config` flag which specifies the path for the configuration file when exporting. For serving the path should be `<model-repository-path>/<model-name>/config.pbtxt` as described in the [docs](https://github.com/triton-inference-server/server/blob/main/docs/model_repository.md#onnx-models). A [schema](utils/serving/model_config.proto) has been compiled into a [python file](utils/serving/model_config_pb2.py) by running `protoc utils/serving/model_config.proto --python_out .` which requires installing `protobuf-compiler`.
