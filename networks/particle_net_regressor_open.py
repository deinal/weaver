...
class ParticleNetRegressor(nn.Module):
    ...
    def forward(
        ...
        pf_features = self.pf_conv(pf_features * pf_mask) * pf_mask
        x = self.pn(pf_points, pf_features, pf_mask)
        x = torch.cat((x, jet_features), dim=1)
        output = self.fc(x)
        return output
...
def get_model(data_config, **kwargs):
    ...
    num_edge_conv_blocks = kwargs.get('num_edge_conv_blocks', 3)
    ...
    conv_params = num_edge_conv_blocks * [(k, num_conv_layers * [conv_dim])]
    ...
    model = ParticleNetRegressor(
        pf_features_dims, jet_features_dims, 
        num_classes, conv_params, fc_params,
