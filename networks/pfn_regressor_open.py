...
class ParticleFlowNetworkRegressor(nn.Module):
    def __init__(self, 
        ...
        **kwargs
        ...
        self.pf_pfn = ParticleFlowNetwork(pf_input_dims, Phi_sizes, use_bn)
        ...
        for i in range(len(F_sizes)):
            ...
            f_layers.append(nn.Sequential(
                nn.Linear(jet_input_dims + Phi_sizes[-1] if i == 0 else F_sizes[i - 1][0], out_units),
                nn.ReLU(),
    ...
    def forward(self, pf_features, pf_mask, jet_features):
        pf_x = self.pf_pfn(pf_features, pf_mask)
        jet_x = jet_features
        x = torch.cat((pf_x, jet_x), dim=1)
        output = self.fc(x)
        return output
...
def get_model(data_config, **kwargs):
    ...
    pf_input_dims = len(data_config.input_dicts['pf_features'])
    jet_input_dims = len(data_config.input_dicts['jet_features'])
    num_classes = 1
    model = ParticleFlowNetworkRegressor(
        pf_input_dims, jet_input_dims, num_classes, 
